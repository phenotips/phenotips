...
/**
 * Tests for the {@link MiddleFingerMeasurementHandler} component, and a few methods from the base @{link
 * {@link AbstractMeasurementHandler} class.
 *
 * @version $Id$
 * @since 1.4
 */
public class MiddleFingerTest
{
    @Rule
    public final MockitoComponentMockingRule<MeasurementHandler> mocker =
        new MockitoComponentMockingRule<MeasurementHandler>(MiddleFingerMeasurementHandler.class);

    private MiddleFingerMeasurementHandler getComponent() throws ComponentLookupException
    {
        return (MiddleFingerMeasurementHandler) this.mocker.getComponentUnderTest();
    }

    @Test
    public void nameIsMidFinger() throws ComponentLookupException
    {
        Assert.assertSame("midFinger", getComponent().getName());
    }
