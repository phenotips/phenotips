 ...
 * @version $Id$
 */
public class PatientJsonExportAuditEventProcessorTest
{
    @Rule
    public final MockitoComponentMockingRule<AuditEventProcessor> mocker =
        new MockitoComponentMockingRule<>(PatientJsonExportAuditEventProcessor.class);
    ...
    private DocumentReference doc = new DocumentReference("wiki", "PhenoTips", "ExportPatient");
    ...
    public void setup() throws ComponentLookupException
    {
        ...
        when(this.request.getParameter("id")).thenReturn("P0000001");

        DocumentReferenceResolver<String> resolver =
            this.mocker.getInstance(DocumentReferenceResolver.TYPE_STRING, "currentmixed");
        when(resolver.resolve("P0000001", new EntityReference("data", EntityType.SPACE))).thenReturn(this.patientDoc);
    }
    ...
    @Test
    public void returnsInputIfNotExportPage() throws ComponentLookupException
    {
        final AuditEvent e = new AuditEvent(this.user, "ip", "get", null,
            new DocumentReference("wiki", "PhenoTips", "ExportFamily"), Calendar.getInstance());
        Assert.assertSame(e, this.mocker.getComponentUnderTest().process(e));
    }

    @Test
    public void returnsExportOnPatient() throws ComponentLookupException
    {
        final AuditEvent e = new AuditEvent(this.user, "ip", "get", "oldExtra", this.doc, Calendar.getInstance());
        ...
        Assert.assertSame("export", result.getAction());
        Assert.assertEquals("json", result.getExtraInformation());
