...
import org.phenotips.vocabulary.translation.internal.SpanishHPOTranslation;
...
/**
 * Tests for the Spanish translation of HPO.
 *
 * @version $Id$
 */
public class SpanishHPOTranslationTest
{
    @Rule
    public final MockitoComponentMockingRule<VocabularyExtension> mocker =
        new MockitoComponentMockingRule<>(SpanishHPOTranslation.class);
        ...
        throws ComponentLookupException, IOException, SolrServerException, CacheException
    {
        ...
        when(this.localizationContext.getCurrentLocale()).thenReturn(new Locale("es"));
        ...
        when(this.config.getProperty("languages", Collections.singleton("es")))
            .thenReturn(Collections.singleton("es"));
    ...
    @Test
    public void spanishFieldTypeIsAddedDuringInitialization() throws JSONException, IOException
    {
        ...
        Assert.assertEquals("text_general_es", request.get("name"));
        Assert.assertEquals("org.apache.lucene.analysis.es.SpanishAnalyzer",
            request.getJSONObject("analyzer").get("class"));
    }

    @Test
    public void spanishFieldsAreAddedDuringInitialization() throws JSONException, IOException
    {
        ...
        Assert.assertTrue(fieldNames.contains("name_es"));
        Assert.assertTrue(fieldNames.contains("def_es"));
        Assert.assertTrue(fieldNames.contains("synonym_es"));
        for (JSONObject request : fieldRequests) {
            Assert.assertEquals("text_general_es", request.get("type"));
    ...
    public void termsAreExtendedWhenLocaleIsSupported()
    {
        ...
        Mockito.verify(this.term).set("name_es", "Todo");
    }
}
