...
import org.phenotips.vocabulary.translation.internal.FrenchHPOTranslation;
...
/**
 * Tests for the French translation of HPO.
 *
 * @version $Id$
 */
public class FrenchHPOTranslationTest
{
    @Rule
    public final MockitoComponentMockingRule<VocabularyExtension> mocker =
        new MockitoComponentMockingRule<>(FrenchHPOTranslation.class);
        ...
        throws ComponentLookupException, IOException, SolrServerException, CacheException
    {
        ...
        when(this.config.getProperty("languages", Collections.singleton("fr")))
            .thenReturn(Collections.singleton("fr"));
    ...
    @Test
    public void frenchFieldTypeIsAddedDuringInitialization() throws JSONException, IOException
    {
        ...
        Assert.assertEquals("text_general_fr", request.get("name"));
        Assert.assertEquals("org.apache.lucene.analysis.fr.FrenchAnalyzer",
            request.getJSONObject("analyzer").get("class"));
    }

    @Test
    public void frenchFieldsAreAddedDuringInitialization() throws JSONException, IOException
    {
        ...
        Assert.assertTrue(fieldNames.contains("name_fr"));
        Assert.assertTrue(fieldNames.contains("def_fr"));
        Assert.assertTrue(fieldNames.contains("synonym_fr"));
        for (JSONObject request : fieldRequests) {
            Assert.assertEquals("text_general_fr", request.get("type"));
    ...
    public void termsAreExtendedWhenLocaleIsSupported()
    {
        ...
        Mockito.verify(this.term).set("name_fr", "Tous");
    }
}
