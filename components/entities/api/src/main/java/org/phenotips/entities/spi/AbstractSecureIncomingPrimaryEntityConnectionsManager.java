...
/**
 * Base class for implementing secure predicates, where the connections are stored in the Objects, referencing the
 * Subject, and each operation checks that the user has edit rights on the affected entities.
 * <p>
...
public abstract class
    AbstractSecureIncomingPrimaryEntityConnectionsManager<S extends PrimaryEntity, O extends PrimaryEntity>
    extends AbstractIncomingPrimaryEntityConnectionsManager<S, O> implements PrimaryEntityConnectionsManager<S, O>
{
    @Inject
    ...
    @Override
    public boolean disconnectAll(S subject)
    {
        if (subject == null) {
            throw new IllegalArgumentException();
        }
        if (!this.authorizationService.hasAccess(this.userManager.getCurrentUser(), Right.EDIT,
            subject.getDocumentReference())) {
            return false;
        }
        return super.disconnectAll(subject);
    }

    @Override
    public boolean disconnectFromAll(O object)
    {
        if (object == null) {
            throw new IllegalArgumentException();
        }
        ...
        return super.disconnectFromAll(object);
    }
}
