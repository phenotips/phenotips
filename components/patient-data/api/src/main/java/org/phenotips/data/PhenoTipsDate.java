...
public class PhenoTipsDate
{
    ...
    {
        // NOTE: this implementation outputs a deprecated (since v1.3m2) JSON to
        //       to make it compatible with 1.2.x UI code

        Integer decadeStart = (this.year == null) ? null : new Double(Math.floor(this.year / 10) * 10).intValue();

        JSONObject result = new JSONObject();
        if (decadeStart != null) {
            result.put(DEPRECATED_JSON_DECADE, decadeStart + "s");
        }
        if (this.rangeYears == null || this.rangeYears == 1) {
            if (this.year != null) {
                result.put(JSON_YEAR_FIELDNAME, this.year.toString());
            }
            if (this.month != null) {
                result.put(JSON_MONTH_FIELDNAME, this.month.toString());
            }
            if (this.day != null) {
                result.put(JSON_DAY_FIELDNAME, this.day.toString());
            }
        }
        return result;
    }
