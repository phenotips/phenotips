...
public class DatesController implements PatientDataController<PhenoTipsDate>
{
        ...
        try {
            ...
            for (String propertyName : getPatientDocumentProperties()) {
                ...
                if (!StringUtils.isEmpty(dateAsEntered)) {
                    ...
                    try {
                        JSONObject dateAsEnteredJSON = JSONObject.fromObject(dateAsEntered);
    ...
    public void save(Patient patient)
    {
        try {
            ...
            for (String propertyName : this.getPatientDocumentProperties()) {
                if (containsKey(dates, propertyName)) {
    ...
    // workaround around 1.2.x not having a containsKey() method for PatientData
    private boolean containsKey(PatientData<PhenoTipsDate> data, String key)
    {
    ...
    public void writeJSON(Patient patient, JSONObject json, Collection<String> selectedFieldNames)
    {
        ...
        for (String propertyName : this.getPatientDocumentProperties()) {
            ...
            // in all other cases need to add the property to JSON, either with empty value
            // (when no date is set or date can not be inferred) or with proper value
            json.put(getJSONFieldName(propertyName),
                    (data == null || data.toEarliestPossibleISODate() == null)
                    ? "" : dateFormat.format(data.toEarliestPossibleISODate()));
        }
    }
