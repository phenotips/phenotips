...
/**
 * Detects new patient records and fires a {@link PatientCreatingEvent} followed by a{@link PatientChangingEvent}.
 *
 * @version $Id$
 * @since 1.0RC1
 */
@Component
@Named("patientCreatingEventSource")
@Singleton
public class PatientCreatingEventSource implements EventListener
{
    @Inject
    ...
    {
        return "patientCreatingEventSource";
    }
    ...
    public void onEvent(Event event, Object source, Object data)
    {
        ...
        CancelableEvent patientEvent = new PatientCreatingEvent(patient, user);
        this.observationManager.notify(patientEvent, doc);
        if (patientEvent.isCanceled()) {
            // FIXME DocumentCancelingEvent is not cancelable yet!
            // ((CancelableEvent) event).cancel();
            // return;
        }
        patientEvent = new PatientChangingEvent(patient, user);
        this.observationManager.notify(patientEvent, doc);
        if (patientEvent.isCanceled()) {
            // FIXME DocumentCancelingEvent is not cancelable yet!
            // ((CancelableEvent) event).cancel();
