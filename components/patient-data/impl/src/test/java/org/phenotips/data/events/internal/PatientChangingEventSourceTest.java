...
public class PatientChangingEventSourceTest
{
    @Rule
    public final MockitoComponentMockingRule<EventListener> mocker =
        new MockitoComponentMockingRule<EventListener>(PatientChangingEventSource.class);

    @Test
    public void hasProperName() throws ComponentLookupException
    {
        Assert.assertEquals("patientChangingEventSource", this.mocker.getComponentUnderTest().getName());
    }

    @Test
    public void listensForDocumentUpdatingEvents() throws ComponentLookupException
    {
        List<Event> events = this.mocker.getComponentUnderTest().getEvents();
        Assert.assertFalse(events.isEmpty());
        Assert.assertEquals(1, events.size());
        Assert.assertTrue(events.iterator().next().matches(new DocumentUpdatingEvent()));
    ...
    @Test
    public void firesPatientChangingEventsWhenPatientDocumentsAreChanging() throws ComponentLookupException
    {
        ...
        this.mocker.getComponentUnderTest().onEvent(new DocumentUpdatingEvent(), doc, null);
    ...
    @Test
    public void doesntFireWhenNonPatientDocumentsAreChanging() throws ComponentLookupException
    {
        XWikiDocument doc = mock(XWikiDocument.class);
    ...
    @Test
    public void doesntFireWhenPatientTemplateIsChanging() throws ComponentLookupException
    {
        XWikiDocument doc = mock(XWikiDocument.class);
        when(doc.getXObject(Patient.CLASS_REFERENCE)).thenReturn(mock(BaseObject.class));
