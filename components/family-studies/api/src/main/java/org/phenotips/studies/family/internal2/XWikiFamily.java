...
public class XWikiFamily implements Family
{
    ...
    public synchronized boolean removeMember(Patient patient)
    {
        ...
        PedigreeUtils.Pedigree pedigree = PedigreeUtils.getPedigree(patientDocument);
        if (pedigree != null && !pedigree.isEmpty()) {
            JSONObject pedigreeData = pedigree.getData();

            /* Should not prevent saving the document */
            try {
                this.stripIdsFromPedigree(pedigreeData, patientDocument.getDocumentReference().getName());

                // TODO: check that pedgreeData changed
                String image = SvgUpdater.removeLinks(pedigree.getImage(), patientId);
                PedigreeUtils.storePedigree(patientDocument, pedigreeData, image, context);
            } catch (Exception e) {
     ...
     */
    private void stripIdsFromPedigree(JSONObject pedigreeData, String patientId)
    {
        List<JSONObject> patientProperties =
            PedigreeUtils.extractPatientJSONPropertiesFromPedigree(pedigreeData);
        for (JSONObject properties : patientProperties) {
            Object patientLink = properties.get(XWikiFamily.PATIENT_LINK_JSON_KEY);
            if (patientLink != null && !StringUtils.equalsIgnoreCase(patientLink.toString(), patientId)) {
                properties.remove(XWikiFamily.PATIENT_LINK_JSON_KEY);
            }
        }
    }
