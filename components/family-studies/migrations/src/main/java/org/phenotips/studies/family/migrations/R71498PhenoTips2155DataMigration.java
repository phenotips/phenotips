...
public class R71498PhenoTips2155DataMigration extends AbstractHibernateDataMigration
{
    ...
    private class MigratePedigreeCallback implements HibernateCallback<Object>
    {
        ...
        private final Set<String> autoLinkingSupportedRelativeTypes = new HashSet<String>(
                Arrays.asList(FAMSTUDIES_PARENT, FAMSTUDIES_CHILD, FAMSTUDIES_SIBLING, FAMSTUDIES_TWIN));

        private final Map<String, String> relativeIdToName =
        ...
        public Object doInHibernate(Session hSession) throws HibernateException, XWikiException
        {
            ...
            try {
                ...
                this.migrator.logger.error("Family Studies migration exception: [{}] [{}]", ex.getMessage(), ex);
            } finally {
        ...
        /**
         * The actual migration work: go patient by patient, find "proband" patients, and create new
         * families using family studies data (when possible) or add comments to existing pedigrees using
         * family studies data (when a pedigree is already present).
         */
        ...
        {
            try {
                this.migrator.logger.debug("Migration: saving document {}...", doc.getDocumentReference().getName());
                ...
                this.migrator.logger.error("Error saving a document: {} - [{}]", ex.getMessage(), ex);
                return false;
            }
        }
                ...
                Map<String, List<String>> patientReferredBy)
        {
            ...
            for (Map.Entry<String, String> relativeEntry : relatives.entrySet()) {
                ...
                if (!autoLinkingSupportedRelativeTypes.contains(relationshipType)) {
        ...
        /**
         * In an existing pedigree finds a node linked to the given patient and adds all given relatives to
         * pedigree node comments.
         *
                ...
                Map<String, String> relativesToAdd)
        {
            try {
                XWikiDocument familyXDocument =
                        this.context.getWiki().getDocument(this.migrator.resolver.resolve(familyId), this.context);

                BaseObject pedigreeObj = familyXDocument.getXObject(Pedigree.CLASS_REFERENCE);
                if (pedigreeObj == null) {
                    this.migrator.logger.error("Error updating existing family: no pedigree object for {}", familyId);
                    return;
                }
                ...
                if (StringUtils.isNotBlank(data.toText())) {
                    ...
                    this.migrator.familyMigrations.updatePedigreeComment(pedigree,
                            patientId, pedigreeCommentAddition);

                    pedigreeObj.set(Pedigree.DATA, pedigree.toString(), context);

                    String comment = this.migrator.getDescription() + " (adding comments to pedigree)";
                    if (!this.saveXWikiDocument(familyXDocument, comment)) {
                        this.migrator.logger.error("Updated family document was not saved for family {}", familyId);
                    }
                }
            } catch (Exception ex) {
                this.migrator.logger.error("Error updating pedigree for family {}: {}", familyId, ex.getMessage());
            }
        }
        ...
        private String generatePedigreeCommentForRelatives(Map<String, String> relativesToAdd)
        {
            ...
            for (Map.Entry<String, String> relativeEntry : relativesToAdd.entrySet()) {
                String relationshipType = relativeEntry.getValue();
                String relativeNamePlusArticle = relativeIdToName.containsKey(relationshipType)
                        ? relativeIdToName.get(relationshipType) : relationshipType;
        ...
        /**
         * Auto-generates a pedigree JSON and creates a family with the generated pedigree linking to the
         * patients that are OK to be linked to (and adds others as comments in the pedigree).
         *
                ...
                Map<String, String> relativesToLink, Map<String, String> relativesToMention)
        {
            try {
                ...
                String documentUpdateDescription = this.migrator.getDescription()
                ...
                if (familyXDocument == null || !this.saveXWikiDocument(familyXDocument, documentUpdateDescription)) {
                    this.migrator.logger.error("Failed to create new family document for patient {}", probandId);
                }

                this.migrator.logger.info("Created new family {} for patient {}",
                        familyXDocument.getDocumentReference().getName(), probandId);

            } catch (Exception ex) {
                this.migrator.logger.error("Error creating new family: [{}] [{}]", ex.getMessage(), ex);
            }
        }

        /**
         * Generates a pedigree JSON.
         *
         * Only relatives from the list ["parent", "child", "sibling", "twin"] may be auto-added
                    ...
                    List<String> twinIDs, List<String> siblingIDs, List<String> childrenIDs, List<String> parentIDs)
        {
            ...
            this.migrator.logger.debug("Created pedigree JSON: [{}]", result.toString());
            return result;
        }
        ...
        /**
         * Figure out which of the parents is the father (if any), which is the mother (if any)
         * and updates node properties accordingly. In case data can be interpreted in multiple ways
         * sets first parent to be the father.
         *
        ...
        /**
         * Creates  anew family document, sets th epedigree to the given one and links given
         * patients ot the family and family to the patients.
         *
         * Only relatives from the list ["parent", "child", "sibling", "twin"] may be auto-added to the
         * generated pedigree.
         */
                ...
                Map<String, String> relativesToLink)
        {
            ...
            this.migrator.logger.debug("Creating new family for patient {}.", probandPatientId);

            XWikiDocument newFamilyXDocument =
                this.migrator.familyMigrations.createFamilyDocument(this.getPatientXDocument(probandPatientId),
        ...
        private void setAllFamilyRefs(Set<String> familyMembers, XWikiDocument familyXDocument, JSONObject probandData)
        {
            ...
            try {
                ...
                for (String memberId : familyMembers) {
                    ...
                    if (!this.migrator.familyMigrations.setFamilyReference(patientDoc, familyDocumentRef, this.context)
                        || !this.saveXWikiDocument(patientDoc, this.migrator.getDescription())) {
                        this.migrator.logger.error("Failed to link patient {} to family {}", memberId, familyID);
                    }
                }
                ...
                this.migrator.logger.error("Failed to link family {} to its members [{}]", familyID, familyMembers);
            }
        }
        ...
        private XWikiDocument getRelativeDoc(String relativeDoc, XWiki xwiki)
        {
            try {
                ...
                rq.setString("patclass", this.migrator.serializer.serialize(Patient.CLASS_REFERENCE));
        ...
        {
            try {
                return this.context.getWiki().getDocument(this.migrator.resolver.resolve(patientId), this.context);
            } catch (Exception ex) {
                return null;
            }
        }
        ...
        private JSONObject getBasicPatientData(XWikiDocument patientDoc)
        {
            ...
            if (patientDoc != null) {
                ...
                result.put(OWNER, this.migrator.familyMigrations.getOwner(patientDoc));
            }
            return result;
        }
    }
