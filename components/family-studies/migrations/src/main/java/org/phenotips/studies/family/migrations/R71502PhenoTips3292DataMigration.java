 ...
 */
@Component
@Named("R71502-PT-3292")
@Singleton
public class R71502PhenoTips3292DataMigration extends AbstractHibernateDataMigration
    implements HibernateCallback<Object>
{
    ...
    private static final String CANCER_NAME_LABEL = "label";
        ...
        throws DataMigrationException, XWikiException, JSONException
    {
        ...
        if (StringUtils.isNotBlank(storedData)) {
            ...
            final JSONObject pedigreeSettings = pedigree.optJSONObject(PEDIGREE_SETTINGS_KEY);
    ...
    private boolean updatePedigreeSettings(@Nullable final JSONObject pedigreeSettings)
    {
        if (pedigreeSettings != null && pedigreeSettings.length() != 0) {
            final JSONObject pedigreeColours = pedigreeSettings.optJSONObject(PEDIGREE_COLORS_FIELD);
            return pedigreeColours != null && updateColours(pedigreeColours);
        }
        return false;
    }
     ...
     *
     * @param pedigreeColours a {@link JSONObject} pedigree colours data
     * @return true iff pedigree colours were updated, false otherwise
     */
    private boolean updateColours(@Nonnull final JSONObject pedigreeColours)
    {
        // A JSONObject mapping cancer names to colours.
        final JSONObject cancerColours = pedigreeColours.optJSONObject(PEDIGREE_CANCERS_FIELD);
        // If no cancer colours recorded, nothing to change.
        if (cancerColours == null || cancerColours.length() == 0) {
            return false;
        }
        // Construct a JSONObject where cancer IDs are mapped to colours (instead of cancer names).
        final JSONObject newCancerColours = new JSONObject();
        // For each cancer -> colour, remap.
        cancerColours.keys()
            .forEachRemaining(cancer
                -> newCancerColours.put(translate(cancer), cancerColours.getString(cancer)));
        // Store the new mapping.
        pedigreeColours.put(PEDIGREE_CANCERS_FIELD, newCancerColours);
        return true;
    }
    ...
    private boolean updateCancers(@Nonnull final Object node)
    {
        ...
        final JSONObject properties = nodeJSON.optJSONObject(PEDIGREE_PROPERTIES_STRING);
        if (properties == null) {
            return false;
        }
        ...
        @Nonnull final JSONArray cancersJsonArray)
    {
        if (cancersJson.length() != 0) {
            ...
            newCancerJson.put(CANCER_NAME_LABEL, cancerName);
