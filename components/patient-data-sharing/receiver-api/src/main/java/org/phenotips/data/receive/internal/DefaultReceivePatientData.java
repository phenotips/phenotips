...
public class DefaultReceivePatientData implements ReceivePatientData
{
    ...
    protected String generateNewToken(String userName)
    {
        ...
        return new BigInteger(256, secureRandomGenerator).toString(32);
    }

    protected static enum TokenStatus { VALID, EXPIRED, INVALID };
    ...
    /**
     * Check that username and credentials (either password or user_token) are received in
     * the request, that {@code username} is a valid user name on this server and that
     * credentials are  valid for the username given.<br>
     * If user_token are enabled on the server and user_token is present in the request
     * then only the token is validated and password is ignored.
     *
     * @return {@code null} iff user name and user credentials are valid, a JSON object containing
        ...
        try {
            ...
            if (guid != null) {
                ...
                if (groupName != null) {
                    ...
                    permisionManager.getPatientAccess(affectedPatient).setOwner(group.getReference());
                    permisionManager.getPatientAccess(affectedPatient).addCollaborator(user.getProfileDocument(), this.permisionManager.resolveAccessLevel("manage"));
                }
                else {
                    permisionManager.getPatientAccess(affectedPatient).setOwner(user.getProfileDocument());
                }
    ...
    public JSONObject getConfiguration()
    {
        try {
		    ...
		    List<String> acceptedFields = this.configurationManager.getActiveConfiguration().getEnabledNonIdentifiableFieldNames();
            ...
            if (this.userTokensEnabled(serverConfig)) {
	            ...
	            if (token == null) {
	                token = generateNewToken(userName);      // Note: if token is not null it must be valid, keep it until expire date
	                                                         // TODO: can be a config option to keep regenerating at each login (and thus keep pushing the expire date)
	            }
    ...
    {
        try {
            Query q = this.queryManager.createQuery("from doc.object(PhenoTips.PatientClass) as o where o.guid = :guid", Query.XWQL)
                      .bindValue("guid", guid);
