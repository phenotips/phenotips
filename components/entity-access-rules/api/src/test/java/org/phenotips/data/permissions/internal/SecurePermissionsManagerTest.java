...
public class SecurePermissionsManagerTest
{
    ...
    public final MockitoComponentMockingRule<PermissionsManager> mocker =
        new MockitoComponentMockingRule<>(SecurePermissionsManager.class);
    ...
    @Test
    public void getEntityAccessReturnsSecureAccess() throws ComponentLookupException
    {
        PermissionsManager internal = this.mocker.getInstance(PermissionsManager.class);
        ...
        EntityAccess result = this.mocker.getComponentUnderTest().getEntityAccess(patient);
    ...
    {
        PermissionsManager internal = this.mocker.getInstance(PermissionsManager.class);
        Collection<Patient> input = Collections.emptyList();
        when(internal.filterVisible(Collections.emptyList(), this.publicVisibility))
            .thenReturn(Collections.emptyList());

        Collection<Patient> result =
            this.mocker.getComponentUnderTest().filterByVisibility(input, this.publicVisibility);
        Mockito.verify(internal).filterVisible(Collections.emptyList(), this.publicVisibility);
    ...
    {
        PermissionsManager internal = this.mocker.getInstance(PermissionsManager.class);
        Iterator<Patient> input = Collections.emptyIterator();
        when(internal.filterVisible(Collections.emptyIterator(), this.publicVisibility))
            .thenReturn(Collections.emptyIterator());

        Iterator<Patient> result = this.mocker.getComponentUnderTest().filterByVisibility(input, this.publicVisibility);
        Mockito.verify(internal).filterVisible(Collections.emptyIterator(), this.publicVisibility);
        Assert.assertSame(input, result);
    }
}
