...
/**
 * Tests for the {@link R71509PhenoTips3476DataMigration}.
 *
 * @version $Id$
 */
public class R71509PhenoTips3476DataMigrationTest
{
    @Rule
    public final MockitoComponentMockingRule<HibernateDataMigration> mocker =
        new MockitoComponentMockingRule<>(R71509PhenoTips3476DataMigration.class);

    /** Sending an event with a non-family document doesn't alter the document. */
    @Test
    public void hibernateMigrate() throws Exception
    {
        ...
        docs.add("Family.hasNoVisibility");
        when(q.list()).thenReturn(docs);
        DocumentReferenceResolver<String> resolver =
            this.mocker.getInstance(DocumentReferenceResolver.TYPE_STRING, "current");

        DocumentReference r1 = new DocumentReference("xwiki", "Family", "hasNoVisibility");
        when(resolver.resolve("Family.hasNoVisibility")).thenReturn(r1);
    ...
    public void hibernateMigrateIgnoresMigrationErrors() throws Exception
    {
        ...
        when(session.createQuery(Matchers.anyString())).thenReturn(q);
        List<String> docs = new ArrayList<>();
        docs.add("Family.ThrowsDME");
        when(q.list()).thenReturn(docs);
        DocumentReferenceResolver<String> resolver =
            this.mocker.getInstance(DocumentReferenceResolver.TYPE_STRING, "current");

        DocumentReference r2 = new DocumentReference("xwiki", "Family", "ThrowsDME");
        when(resolver.resolve("Family.ThrowsDME")).thenReturn(r2);
    ...
    {
        Assert.assertEquals(71509, this.mocker.getComponentUnderTest().getVersion().getVersion());
    }
