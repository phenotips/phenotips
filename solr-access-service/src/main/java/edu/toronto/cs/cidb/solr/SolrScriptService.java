    ...
    private SolrDocumentList search(MapSolrParams params)
    {
        try {
            ...
            if (results.size() == 0 && !response.getSpellCheckResponse().isCorrectlySpelled()) {
                ...
                // The spellcheck doesn't preserve the identifiers, manually correct this
                suggestedQuery = suggestedQuery.replaceAll("term_category:hip", "term_category:HP");
                MapSolrParams newParams = new MapSolrParams(getSolrQuery(
                    suggestedQuery, params.getInt(CommonParams.ROWS, -1), params.getInt(CommonParams.START, 0)));
